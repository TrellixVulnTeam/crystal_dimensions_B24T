...
class Story:
    ...
    def validate_files(self):
        ...
        file_list = list(
            map(
                os.path.basename,
                glob.glob("adventure/" + self.story_directory + "/*.yaml"),
            )
        )
        ...
        if story_files.sort() == file_list.sort():
            ...
            msg = (
                "You have not added all the correct files to your story theme"
            )
            return msg
    ...
    def get_player_characters(self):
        with open(
            "adventure/" + self.story_directory + "/player_characters.yaml"
        ) as f:
    ...
    def get_destinations(self):
        with open(
            "adventure/" + self.story_directory + "/destinations.yaml"
        ) as f:
            destinations = yaml.load(f, Loader=yaml.FullLoader)
        return destinations

    def get_zones(self):
        with open("adventure/" + self.story_directory + "/zones.yaml") as f:
            zones = yaml.load(f, Loader=yaml.FullLoader)
        return zones

    def get_transport(self):
        with open(
            "adventure/" + self.story_directory + "/transport.yaml"
        ) as f:
            transport = yaml.load(f, Loader=yaml.FullLoader)
        return transport

    def get_shop(self):
        with open("adventure/" + self.story_directory + "/shop.yaml") as f:
            shop = yaml.load(f, Loader=yaml.FullLoader)
        return shop

    def get_npc(self):
        with open(
            "adventure/" + self.story_directory + "/non_player_characters.yaml"
        ) as f:
            non_player_characters = yaml.load(f, Loader=yaml.FullLoader)
        return non_player_characters

    def get_objects(self):
        with open("adventure/" + self.story_directory + "/objects.yaml") as f:
            objects = yaml.load(f, Loader=yaml.FullLoader)
        return objects

    def get_items(self):
        with open("adventure/" + self.story_directory + "/items.yaml") as f:
