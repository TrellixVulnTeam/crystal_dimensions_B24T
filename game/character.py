...
class Hero(Character):
    ...
    def show_navigator(self, game):
        ...
        game.msg.append(
            str("Vehicle category: {category}").format(
                category=category.upper()
            )
        )
        game.msg.append(str("Fuel: {fuel}").format(fuel=fuel_tank))
        game.msg.append(
            str("Current location: {location}").format(
                location=self.destination.name.upper()
            )
        )
        game.msg.append(
            str("Zone: {zone}").format(zone=self.zone.name.upper())
        )
    ...
    def go(self, game, command):
        ...
        go_styles = [
            "swagger over to the",
            "jog over to the",
            "walk cautiously in the direction of the",
            "sprint towards the ",
            "do a ninja roll to get to the",
        ]

        if command in go_directions:
            game.player.zone = game.player.destination.zones[
                go_directions[command]
            ]
            game.msg.append(
                str("Travelling {direction} you reach the {zone}").format(
                    direction=command.title(), zone=game.player.zone.name
                )
            )
            ...
            game.msg.append(
                str("You {go} {object}").format(
                    go=go_style, object=game.player.zone.objects.name
                )
            )
    ...
    def look(self, game, command):
        if command == "closer" and "look" in game.player.moves:
            ...
            if game.player.zone.items.collected == False:
                if game.player.zone.items.collected == False:
                    game.msg.append(
                        "see that the item in front of you looks a lot like a "
                        + game.player.zone.items.name
                    )
            ...
            if game.player.zone.winning_items != "":
                if game.player.zone.winning_items.collected == False:
                    game.msg.append(
                        "Wow!! You see what appears to be a "
                        + game.player.zone.winning_items.name
                    )
                    game.msg.append(game.player.zone.winning_items.description)
            else:
                game.msg.append(
                    "...but there doesn't appear to be anything here to see..."
                )
        elif command == "around":
            if (
                game.player.zone == game.destinations[0].zones[0]
                and game.player.weapon == ""
            ):
                game.select_weapon()
            else:
                ...
                if isinstance(game.player.zone.winning_items, WinningItem):
                    if game.player.zone.winning_items.collected == False:
                        game.msg.append(
                            game.player.zone.winning_items.msg["look"]
                        )
    ...
    def list_inventory(self, game, command, task):
        if task == "list":
            ...
            if self.inventory:
                ...
                game.msg.append(
                    str(
                        "Remember you can only hold {capacity} items in your pack"
                    ).format(capacity=pack_max)
                )
            else:
...
class NonPlayerCharacter(Character):
    ...
    def encounter(self, game):
        ...
        game.msg.append(
            str(
                """
It's {name} from the species {species}"""
            ).format(name=self.name, species=self.species)
        )
        game.msg.append(self.appearance.title())
        game.msg.append(
            str(
                """You know of these guys and you know them to be {status}"""
    ...
    def attack(self, game):
        ...
        print(
            str(
                """{name} are you ready to commence battle with {npc}?"""
            ).format(name=game.player.name, npc=self.name)
        )
        ...
        if attack == "y":
            ...
            if game.player.weapon:
                print(
                    str(
                        """You have your trusty {weapon} - this will add strength to your attack"""
                    ).format(weapon=game.player.weapon.name)
                )
            ...
            if game.player.health >= game.player.full_health:
                powers = input(
                    str(
                        "You have full health - do you want to use {power} to fight {npc}? (y/n) >"
                    ).format(power=game.player.power, npc=self.name)
                )
                # player strength
                if powers == "y":
                    player_strength + 2
        ...
        while attack == "y" and game.player.health > 0:
            ...
            if npc_attack_pts > player_attack_pts:
                ...
                if game.player.health == 1:
                    game.msg.append(
                        "Your health is on it's last legs - lose one more and you are dead...just saying!"
                    )
                ...
                if game.player.health == 0:
                    game.msg.append(
                        "Oh no you have just died at the hands of " + self.name
                    )
            ...
            if self.health > 0:
                print(
                    "No worries - there is indeed great strength in not fighting."
                )
                time.sleep(1)
