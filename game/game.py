    ...
    def help(self):
        ...
        for command in self.commands:
            if self.commands[command]["hidden"] is False:
                self.msg.append(
                    command + " [ " + self.commands[command]["input"] + " ]"
                )
...
{intro}"""
                ...
                name=self.player.name, power=self.player.power, intro=intro
            )
        )
    ...
    def show_status(self):
        ...
        if self.player.destination.artwork is not None:
            a = open(
                "story/crystal_dimensions/" + self.player.destination.artwork,
                "r",
            )
            artwork = a.read()
            a.read()
        ...
        if not self.player.moves and (
            self.player.destination == self.destinations[0]
        ):
            self.show_welcome()
        ...
        if self.player.health > 0 and (
            (self.player.zone.items == False) and not self.msg
        ):
            self.msg.append(
                str(
                    """
...
on the ground in front of you.
    ...
    def players(self):
        ...
        for key, value in self.player_characters.items():
            print(
                str(key) + ") " + value["name"] + " ( " + value["power"] + " )"
            )
    ...
    def over(self):
        ...
        self.msg.append(
            "Hey - Im afraid you're not longer with us. Type restart to play again."
        )

    def select_weapon(self):
        print(
            """There's a big stack of weapons on the floor in front of you!"""
        )
        i = 1
        ...
        if int(weapon_select) in items["weapons"]:
            self.player.weapon = self.player.zone.weapons[
                int(weapon_select) - 1
            ]
        ...
        if self.player.health <= 0:
            ...
            if command[0] == "check":
                ...
                if command[1] == "location":
                    self.msg.append(
                        str("""Your location: {destination}, {zone}""").format(
                            destination=self.player.destination.name.upper(),
                            zone=self.player.zone.name.upper(),
                        )
                    )
                elif (
                    self.player.list_inventory(self, command[1], "check")
                    == True
                ):
            ...
            if command[0] == "fly":
                ...
                if "spaceship" in self.player.moves:
                    ...
                    if not self.player.transport:
                        self.msg.append(
                            "But it must be your lucky day as you happen to have a "
                        )
                        self.show_vehicle()
                        self.msg.append(
                            str("\ntip: go {transport}\n").format(
            ...
            if command[0] == "get":
                ...
                if len(command) > 1:
                    if command[1]:
                        if (
                            command[1].lower()
                            == self.player.zone.items.name.lower()
                        ):
                            self.player.zone.items.get(self, command[1])
                        elif (
                            isinstance(
                                self.player.zone.winning_items, WinningItem
                            )
                            and command[1].lower()
                            == self.player.zone.winning_items.name.lower()
                        ):
                            if (
                                self.player.zone.non_player_characters.health
                                > 0
                            ):
                                self.player.zone.non_player_characters.encounter(
                                    self
                                )
                            else:
                                self.player.zone.winning_items.get(
                                    self, command[1]
                                )
                        else:
                            self.msg.clear()
            ...
            if command[0] == "fight":
                ...
                self.player.moves.append(command[0])
                if (
                    self.player.zone.non_player_characters
                    and self.player.zone.non_player_characters.health > 0
                ):
                    # launch an attack
            ...
            if command[0] == "buy":
                ...
                self.player.moves.append(command[0])
                if (
                    self.player.zone.objects.name.lower() == "shop"
                    and "shop" in self.player.moves
                ):
                    self.player.zone.objects.buy(self)
                else:
                    self.msg.clear()
                    self.msg.append(
                        "Sorry, but you can't do that here - you need a shop!"
                    )
            # end buy
            ...
            if command[0] == "place":
                ...
                self.player.moves.append(command[0])
                if (
                    self.destinations[-1].zones[-1] == self.player.zone
                    and self.player.list_inventory(self, command[1], "check")
                    == True
                ):
