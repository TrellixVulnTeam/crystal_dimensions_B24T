    ...
    def voyage_list(self, game, task):
        if task == "list":
            ...
            for direction, value in self.voyages.items():
                game.msg.append(
                    str(destinations[value]["name"])
                    + str(" (direction: " + direction.upper() + ")")
                )
    ...
    def voyage(self, game, command):
        ...
        if game.player.transport.fuel_tank >= 10:
            ...
            if command in game.player.destination.voyages:
                ...
                # spend the fuel
                game.player.transport.fuel_tank = (
                    game.player.transport.fuel_tank
                    - game.player.transport.fuel_usage
                )

                game.msg.append(
                    str(
                        """After a quick interstellar hop through the universe
...
"""
                ...
                game.msg.append(
                    "You can't go in that direction - you'll get lost!"
                )
        else:
            game.msg.append(
                "You don't have enough fuel to make the trip dude..."
            )
    ...
    def open(self, game):
        if game.player.list_inventory(self, "key", "check") == True:
            if self.clue:
                game.msg.clear()
                game.msg.append(
                    """The key fits, you turn it anti-clockwise...
...
"""
                ...
                game.msg.clear()
                game.msg.append(
                    str("""Alas the {item} is empty""").format(item=self.name)
                )
        else:
            self.msg.append(
                str(
                    """You don't have anything to open {item} with...perhaps you need to find a key"""
...
"""
            ...
            game.msg.clear()
            game.msg.append(
                str("""Alas there's nothing to see up there...""").format(
                    item=self.name
                )
            )
    ...
    def read(self, game):
        if self.clue:
            game.msg.clear()
            game.msg.append(
                """Taking a step back you start to read the words one by one...
"""
            )
            self.clue.read(game)
        else:
            game.msg.clear()
            game.msg.append(
                str("""Alas not a language you understand...""").format(
                    item=self.name
                )
            )
...
class Shop(Object):
    ...
    def buy(self, game):
        if game.player.credits > 1:
            ...
            for product, product_att in self.products.items():
                print(
                    str("{id}) {name} - (CR: {credits})").format(
                        id=product,
                        name=product_att["name"],
                        credits=product_att["credits"],
                    )
                )
            print("0) Nothing")
            ...
            if int(product_select) in self.products:
                if (
                    game.player.credits
                    >= self.products[int(product_select)]["credits"]
                ):
                    game.player.credits -= self.products[int(product_select)][
                        "credits"
                    ]
                    if self.products[int(product_select)]["name"] == "fuel":
                        game.player.inventory.append(
                            Item(
                                "fuel tank",
                                "Not delicious but fairly nutritious",
                                {
                                    "food": """Actually quite filling - you feel ready for action!!"""
                                },
                                "fuel",
                                12,
                                True,
                        ...
                        game.player.inventory.append(
                            Food(
                                "water bottle",
                                "Always good to quench your thirst",
                                {
                                    "drink": """Pretty delish - not thirsty anymore!"""
                                },
                                "drink",
                                2,
                                True,
            ...
            game.msg.append(
                "You need some crrrr's to buy stuff around here!!?!*!"
            )
    ...
    def sell(self, game):
        ...
        item_select = input(">")
        if (
            game.player.list_inventory(game, item_select.lower(), "check")
            == True
        ):
            ...
            if check == "y":
                for item in game.player.inventory:
                    if item.name.lower() == item_select.lower():
                        ...
                        game.msg.append(
                            str(
                                """You have earnt yourself {credits}"""
...
class Item:
    ...
    def get(self, game, command):
        if command.lower() == self.name.lower():
            ...
            if pack_total < pack_max:
                ...
                game.msg.append(
                    "You can't pick up this item as your pack is full!!"
                )
        else:
    ...
    def drop(self, game, command):
        ...
        if check == "y":
            ...
            game.msg.append(
                str(
                    """You fling the {item} to the ground, freeing up some space in your pack"""
                ).format(item=self.name)
            )
            game.msg.append("\n")
            # show updated list of pack items
...
class Food(Item):
    # Create edible items
    def __init__(
        self, name, description, msg, category, value, collected, health
    ):
        super().__init__(name, description, msg, category, value, collected)
        self.health = health
    ...
    def eat(self, game, command):
        if command == self.name and self.category == "food":
            ...
            if self.health > 0:
                ...
                game.player.health = game.player.health + self.health
                game.msg.append(
                    "Ouch that food was junk dude - you lose some health."
                )
            game.player.inventory.remove(self)
        else:
            game.msg.clear()
...
class Magic(Food):
    ...
    def __init__(
        ...
        super().__init__(
            name, description, msg, category, value, health, collected
        )
        self.spell = spell
...
class Key(Item):
    ...
    def __init__(
        self, name, description, msg, category, value, collected, object=None
    ):
        super().__init__(name, description, msg, category, value, collected)
        if object is None:
            self.object = []
...
class Credit(Item):
    ...
    def get(self, game, command):
        if command.lower() == self.name.lower():
            ...
            game.msg.append(
                str(
                    "You've just added {value} credits to your wallet!"
...
class Clue:
    ...
    def read(self, game):
        if self.status == "unread":
            clue_discovery_msg = {
                "Looks suspiciously like a clue...",
                "Just what you've looking for, a hint",
                "Finding anything around here was going to be tricky without a clue...",
            }
            game.msg.append(random.choice(list(clue_discovery_msg)))
            game.msg.append(
                str(
                    """It's going to be important to get to THE {zone} on your travels"""
                ).format(zone=self.zone.name)
            )
        else:
            game.msg.append(
                """It looks like you already read and binned this clue"""
            )
