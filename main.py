...
# select an adventure (validate files/ folders and print list of available adventures)
if not story_choice:
    cprint("The following adventures are availble to play:", "red")

    cprint("Please choose a story (by entering the number):", "green")
    story_choice = input(">")
...
game_transport = Transport(
    new_story.transport[0]["name"],
    new_story.transport[0]["description"],
    new_story.transport[0]["category"],
    new_story.transport[0]["capacity"],
    new_story.transport[0]["fuel_tank"],
    new_story.transport[0]["fuel_usage"],
    new_story.transport[0]["weapon"],
    new_story.transport[0]["msg"],
)
...
for game_non_player_character in new_story.non_player_characters:
    if (
        new_story.non_player_characters[game_non_player_character]["status"]
        == "boss"
    ):
        game_boss = Boss(
            new_story.non_player_characters[game_non_player_character]["name"],
            new_story.non_player_characters[game_non_player_character][
                "species"
            ],
            new_story.non_player_characters[game_non_player_character][
                "appearance"
            ...
            new_story.non_player_characters[game_non_player_character]["msg"],
        )
    else:
        game_non_player_characters.append(
            NonPlayerCharacter(
                new_story.non_player_characters[game_non_player_character][
                    "name"
                ],
...
game_shop = Shop(
    new_story.shop[0]["name"],
    new_story.shop[0]["description"],
    "buy/sell",
    "",
    new_story.shop[0]["msg"],
    new_story.shop[0]["products"],
)

# winning items
for winning_item in new_story.items["winning_items"].items():
    game_winning_items.append(
        WinningItem(
            winning_item[1]["name"],
            winning_item[1]["description"],
            winning_item[1]["msg"],
            winning_item[1]["category"],
            winning_item[1]["value"],
            False,
        )
    )
...
for game_object in new_story.objects:
    game_objects.append(
        Object(
            new_story.objects[game_object]["name"],
            new_story.objects[game_object]["description"],
            new_story.objects[game_object]["move"],
            "",
            new_story.objects[game_object]["msg"],
        )
    )
...
for game_zone in new_story.zones:
    ...
    game_zones.append(
        Zone(
            new_story.zones[game_zone]["name"],
            zone_object,
            zone_item,
            "",
            new_story.zones[game_zone]["scenario"],
        )
    )
...
for game_destination in new_story.destinations:
    game_destinations.append(
        Destination(
            new_story.destinations[game_destination]["name"],
            new_story.destinations[game_destination]["voyages"],
            new_story.destinations[game_destination]["scenario"],
            new_story.destinations[game_destination]["colour"],
            new_story.destinations[game_destination]["artwork"],
            game_zones_group[i],
        )
    )
    i += 1
    ...
    if not new_game.player:
        ...
        while not new_game.player:
            if int(character) in new_game.player_characters:
                # add player to game
                new_game.player = Hero(
                    new_game.player_characters[int(character)]["name"],
                    new_game.player_characters[int(character)]["power"],
                    "",
                    new_game.player_characters[int(character)]["strength"],
                    new_game.player_characters[int(character)]["speed"],
                    new_game.player_characters[int(character)]["health"],
                    new_game.player_characters[int(character)]["health"],
                    "",
                    "",
                    inventory,
                ...
                new_game.player.inventory.append(
                    Item(
                        "Navigator",
                        "Handheld maps magic",
                        "Your faithful navigation device",
                        "tech",
                        25,
                        True,
                    )
        ...
        while (
            command == ""
            and new_game.player.health > 0
            and len(new_game.player.winning_items)
            != len(new_story.items["winning_items"])
        ):
            command = input(">")
